...
void main() {
	...
	float thisRadius[CNT];
	float iPct[CNT];
	float segmentDistance[CNT];
	float dMin[CNT];
	float rad[CNT];
	float tail[CNT];
	float edge[CNT];
	...
	for(int i=0;i < CNT; i++){
		...
		if(layer4enabled){
			...
			if(i < elements4) {
				...
				if(time_4[i] >= 0.0 && time_4[i] < 0.25){
					...
					rad[i] = (radius * modT + 0.001) * 0.8;
					tail[i] = tailLength * modT + 0.001;
					edge[i] = edgeWidth * modT + 0.001;
				...
				if(time_4[i] >= 0.75 && time_4[i] <= 1.0){
					...
					rad[i] = (radius * (1.0-modT))*0.9;
					tail[i] = tailLength * (1.0-modT);
					edge[i] = edgeWidth * (1.0-modT);
				...
				thisRadius[i] = rad[i] + sin(float(i) * 1.0 + TIME * 1.00) * 0.01 - 0.01;
				//thisRadius[i] = 0.0;
				
				dMin[i] = 1.0;
				
					//how many balls in the trail
					for (int k = 0; k < iMax; k++) {		
						iPct[i] = float(k) / float(iMax);
						segmentDistance[i] = length(thingPosition(TIME * 1.8 + float(i) * 1.0 - iPct[i] * tail[i], aspect) - uv);
						dMin[i] = min(dMin[i], segmentDistance[i] + pow(iPct[i], 0.8) * (thisRadius[i] + edge[i]));
					}
					
				//color_4 = vec3(0.0);
				vec3 mixColor = mix(color1.rgb, color2.rgb, mod(float(i), 2.0));
				float mixBlur = (1.0 - smoothstep(thisRadius[i], thisRadius[i] + edge[i], dMin[i]));

				color_4 += 5.0 * mixBlur * mixColor;
		...
		vec3 outColor = color_1*pickedColor1 + color_2*pickedColor2 + color_3*pickedColor3;


		//know this works!
		//gl_FragColor = vec4(vec3(1.0) - color_4, 1.0);
		
		//mix them together
		//outColor = max(outColor, color_4);
		outColor = mix(outColor, color_4, 0.5);
		
		//mix(outColor, vec3(1.0),0.5);
		
		
		//this has a faded effect
		//gl_FragColor = vec4(outColor, 0.8)+vec4(0.15);
...
vec2 thingPosition(float t, float aspect) {
    ...
    //vec2 p = vec2(sin(PI*0.1 * tx) - cos(PI+0.2 * tx), cos(PI*0.5 * t) + sin(-PI*0.5 * t));
    p.y *= 0.23;
    p.x *= 0.23;    
 	return p;
}